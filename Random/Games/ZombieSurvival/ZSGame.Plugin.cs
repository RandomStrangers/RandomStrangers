...
using RandomStrangers.Events.EconomyEvents;
using RandomStrangers.Events.EntityEvents;
        ...
        static bool WarnPillaring(Player p, ZSData data, ushort x, ushort y, ushort z, bool nonReplacable) {
            if ((!nonReplacable && data.BlocksStacked == 2) || (nonReplacable && data.BlocksStacked == 1)) {
                ...
                if (!data.PillarFined) {
                    Chat.MessageFromOps(p, "  &cWarning: λNICK &Sis pillaring!");
#if DEV_BUILD_RS
                    Command.Find("Take").Use(Player.Random, p.name + " 10 Auto fine for pillaring");
#else
...
#if DEV_BUILD_RS
                    ModAction action = new ModAction(p.name, Player.Random, ModActionType.Kicked, "Auto kick for pillaring");
#else
