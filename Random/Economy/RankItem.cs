...
#if DEV_BUILD_RS
            LevelPermission perm = Group.ParsePermOrName(args[2], LevelPermission.Random);
            if (perm == LevelPermission.Random) return;
#else
...
#if DEV_BUILD_RS
            Command.Find("SetRank").Use(Player.Random, p.name + " " + rank.Name);
#else
