 ...
 */
using System;
using RandomStrangers.Commands.Chatting;

namespace RandomStrangers.Core
{
    ...
    public static class ChatHandler
    {
            ...
            public static void HandleCommand(Player p, string cmd, string args, CommandData data)
        {
            ...
            if (cmd.ToLower() == "pony")
            {
                ...
                int used = p.Extras.GetInt("RS_PONY");
                ...
                p.Extras["RS_PONY"] = used + 1;
            }
            else if (cmd.ToLower() == "rainbowdashlikescoolthings")
            {
                p.cancelcommand = true;
                if (!MessageCmd.CanSpeak(p, cmd)) return;
                int used = p.Extras.GetInt("RS_RD");
                ...
                p.Extras["RS_RD"] = used + 1;
            }
            if (!Server.Config.MCLawlSecretCommands) return;
            if (cmd.ToLower() == "care")
            {
                p.cancelcommand = true;
                int used = p.Extras.GetInt("RS_CARE");
                ...
                p.Extras["RS_CARE"] = used + 1;
            }
            else if (cmd.ToLower() == "facepalm")
            {
                p.cancelcommand = true;
                int used = p.Extras.GetInt("RS_FACEPALM");
