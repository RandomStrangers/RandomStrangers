...

{
    ...
    public static class UIHelpers
    {
        ...
        public static void RepeatCommand()
        {
            if (lastCMD.Length == 0)
            {
                Logger.Log(LogType.CommandUsage, "(Random): Cannot repeat command - no commands used yet.");
                return;
            }
        ...
        public static void HandleCommand(string text)
        {
            ...
            {
                Logger.Log(LogType.CommandUsage, "(Random): Whitespace commands are not allowed.");
                return;
            }
            ...
            if (cmd == null)
            {
                Logger.Log(LogType.CommandUsage, "(Random): Unknown command \"{0}\"", name); return;
            }
            if (!cmd.SuperUseable)
            {
                Logger.Log(LogType.CommandUsage, "(Random): /{0} can only be used in-game.", cmd.name); return;
            }
            ...
            Thread thread = new Thread(
                () => {
                    try
                    {
                        cmd.Use(Player.Random, args);
                        if (args.Length == 0)
                        {
                            Logger.Log(LogType.CommandUsage, "(Random) used /" + cmd.name);
                        }
                        else
                        {
                            Logger.Log(LogType.CommandUsage, "(Random) used /" + cmd.name + " " + args);
                        }
                    }
                    catch (Exception ex)
                    {
                        Logger.LogError(ex);
                        Logger.Log(LogType.CommandUsage, "(Random): FAILED COMMAND");
                    }
                });
