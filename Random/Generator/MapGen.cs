...
using RandomStrangers.Generator.fCraft;
using RandomStrangers.Generator.Realistic;
using ClassicalSharp.Generator;

namespace RandomStrangers.Generator 
{
    ...
    public sealed class MapGen 
    {
        ...
        public static void SetRealmPerms(Player p, Level lvl)
        {
            ...
            const LevelPermission rank = LevelPermission.Random;
            lvl.BuildAccess.Whitelist(Player.Random, rank, lvl, p.name);
            lvl.VisitAccess.Whitelist(Player.Random, rank, lvl, p.name);

            Group grp = Group.Find(Server.Config.OSPerbuildDefault);
            if (grp == null) return;

            lvl.BuildAccess.SetMin(Player.Random, rank, lvl, grp);
        }
    }
}
#else
