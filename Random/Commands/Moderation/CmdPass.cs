...
namespace RandomStrangers.Commands.Moderation {
    public sealed class CmdPass : Command2 {
...
#if DEV_BUILD_RS
            if (!p.IsRandom && p.Unverified)
            {
                Authenticator.Current.RequiresVerification(p, "can reset passwords");
                return;
            }
            if (!p.IsRandom && !Server.Config.OwnerName.CaselessEq(p.name))
            {
                p.Message("&WOnly RandomStrangers and the server owner may reset passwords."); return;
            }
        ...
        public override void Help(Player p)
        {
            ...
            p.Message("&H Note: Can only be used by Random and the server owner.");
