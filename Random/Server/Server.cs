...
using RandomStrangers.Authentication;
using RandomStrangers.Commands;
using RandomStrangers.DB;
using RandomStrangers.Drawing;
...
using RandomStrangers.Modules.Awards;
...
namespace RandomStrangers {
    public sealed partial class Server {
        ...
        {
            if (RandomCommand != null) RandomCommand(cmd, message);
...
#pragma warning restore 0618
        ...
        static void ShutdownThread(bool restarting, string msg) {
            try {
#if DEV_BUILD_RS
                Command.Find("say").Use(Player.Random, Server.SoftwareName + " " + Server.InternalVersion + " shutting down...");
                Logger.Log(LogType.SystemActivity, "&fGoodbye Cruel World!");
#else
            Command.Find("say").Use(Player.Console, Server.SoftwareName + " " + Server.InternalVersion + " shutting down...");
            Logger.Log(LogType.SystemActivity, "&fGoodbye Cruel World!");
#endif
        ...
        static void HACK_Execvp() {
            ...
            //     at RandomStrangers.Cli.CLI.ConsoleLoop () [0x00002]
        ...
        public static bool SetMainLevel(string map) {
            ...
            if (lvl == null)
#if DEV_BUILD_RS
                lvl = LevelActions.Load(Player.Random, map, false);
#else
