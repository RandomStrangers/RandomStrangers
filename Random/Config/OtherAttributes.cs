...
namespace RandomStrangers.Config {
    ...
    public sealed class ConfigPermAttribute : ConfigAttribute {
        ...
        public override object Parse(string raw) {
#if DEV_BUILD_RS
            LevelPermission perm = Group.ParsePermOrName(raw, LevelPermission.Random);
            if (perm > LevelPermission.Random)
            {
            ...
            if (perm > LevelPermission.Random)
            {
                Logger.Log(LogType.Warning, "Config key \"{0}\" cannot be above RandomStrangers.", Name);
                perm = LevelPermission.Random;
            }
            return perm;
        }
#else
