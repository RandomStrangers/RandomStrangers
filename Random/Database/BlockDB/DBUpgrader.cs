...
namespace RandomStrangers.DB {
    ...
    public static class DBUpgrader {
        ...
        public static string[] CompactMessages = new string[] {
            ...
            "   a) doing VACUUM on the database (note that this will create a temp file as big as RandomStrangers.db)",
            "   b) doing /server backup litedb, shutting down the server, deleting RandomStrangers.db, then finally running /server import SQL",
        };
...
#if DEV_BUILD_RS
            Player.Random.MessageLines(CompactMessages);
#else
