...
using RandomStrangers.UI;

namespace RandomStrangers.Cli {
    public static class Program {
        ...
        public static void Main(string[] args) {
            ...
            // If RandomStrangers_.dll is missing, a FileNotFoundException will get thrown for RandomStrangers dll
            try {
                EnableCLIMode();
            } catch (FileNotFoundException) {
                Console.WriteLine("Cannot start server as RandomStrangers_.dll is missing from " + Environment.CurrentDirectory);
            ...
            // separate method, in case RandomStrangers_.dll is missing
            StartCLI();
        }
        ...
        static void EnableCLIMode() {
            try {
                ...
                // in case user is running CLI with older RandomStrangers dll which lacked CLIMode field
            }
...
#if DEV_BUILD_RS
            if (!Server.Config.RandomLogging[(int)type]) return;
#else
        ...
        static void LogNewerVersionDetected(object sender, EventArgs e) {
            Write("&cRandomStrangers update available! Update by replacing with the files from " + Updater.UploadsURL);
        }
