    ...
    public sealed class GamesHelper {
...
#if DEV_BUILD_RS
            if (!game.Running) game.Start(Player.Random, "", int.MaxValue);
#else
...
#if DEV_BUILD_RS
                RoundsGameConfig.AddMap(Player.Random, map, lvlCfg, game);
#else
        ...
        void DelMap_Click(object sender, EventArgs e) {
            try {
                ...
                LevelConfig lvlCfg = LevelInfo.GetConfig(map);
#if DEV_BUILD_RS
                RoundsGameConfig.RemoveMap(Player.Random, map, lvlCfg, game);
#else
