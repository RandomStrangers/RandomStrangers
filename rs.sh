...
rsdir="./"							##  Folder containing RandomStrangers.exe
...
gamename="RedCraft - RandomStrangers"		##  Arbitrary name of server, will not affect actual server name
gamepid="${rsdir}/rs.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${rsdir}/rs.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${rsdir}/RandomStrangers.exe" ]; then
	echo -e "cannot find ${rsdir}/RandomStrangers.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${rsdir}/RandomStrangers.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${rsdir}/RandomStrangers.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${rsdir}/RandomStrangers.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${rsdir}/RandomStrangers.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${rsdir}/RandomStrangers.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
		if [ -f "${rsdir}/RandomStrangers_.update" ]; then
			if [ -f "${rsdir}/RandomStrangers.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${rsdir}/RandomStrangers.exe
				rm ${rsdir}/RandomStrangers_.dll
				mv ${rsdir}/RandomStrangers.update ${rsdir}/RandomStrangers.exe
				mv ${rsdir}/RandomStrangers_.update ${rsdir}/RandomStrangers_.dll
				if [ -f "${rsdir}/RandomStrangers_.update" ]; then
					if [ -f "${rsdir}/RandomStrangers.update" ]; then
						echo -e FAILED!
						if [ -f "${rsdir}/RandomStrangers_.dll" ]; then
							if [ -f "${rsdir}/RandomStrangers.exe" ]; then
								$0 ${default}
							fi
						else
